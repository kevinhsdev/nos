...
const allScenes = ['boot','picker','hello','timeline','mapa','counter','galleryScene','sonar','lua','girassol','reasons','lock','foto','turn','verses','letter','amada','breath','ask','final','roadmap','ingresso','dunamis','contrato','deploy','creditos'];
const hasPlaylist = (Array.isArray(CONFIG.playlist) && CONFIG.playlist.length>0) || !!CONFIG.musica;
const scenes = allScenes.filter(s => {
  if(s==='dunamis') return !!(CONFIG.dunamis && CONFIG.dunamis.ativo);
...
function depthFor(name){
  const map={boot:0.10,picker:0.13,hello:0.16,timeline:0.22,mapa:0.28,counter:0.34,galleryScene:0.40,sonar:0.41,lua:0.43,girassol:0.46,reasons:0.50,lock:0.54,foto:0.60,turn:0.66,verses:0.70,letter:0.74,amada:0.80,breath:0.84,ask:0.95,final:1.0,roadmap:0.82,ingresso:0.815,dunamis:0.81,contrato:0.80,deploy:0.78,creditos:0.70};
...
function runBoot(){
  const screen=$('#bootLines'); let i=0; const promptStr='nós@kevin:~$ ';
...
function launchReady(){
  ...
  run.innerHTML='<span class="prompt">nós@kevin:~$</span> <span class="run">./executar.sh</span> <span class="cur"></span>';
...
function launch(){
  ...
  ac(); yaySfx(); buzz(30);
  document.body.classList.add('launched'); updateStatus(); initSea();
...
let dunamisUnlocked=false, secretMarker=null, secretPinCfg=null;
function initMap(){
  ...
  CONFIG.mapa.pinos.forEach(p=>{
    ...
    if(p.tipo==='segredo') useIcon = dunamisUnlocked ? iconFut : iconSeg;
    const mk=L.marker(p.coord,{icon:useIcon}).addTo(mapObj);
    if(p.tipo==='segredo'){
      secretMarker=mk; secretPinCfg=p;
      const shown = (dunamisUnlocked && p.revelado) ? p.revelado : p;
...
function unlockDunamisPin(){
  dunamisUnlocked=true;   // fica marcado: se ela voltar ao mapa, já aparece destravado
...
$('#yesBtn').addEventListener('click', ()=>{
  ...
  show('final'); yaySfx(); buzz([40,30,80]); celebrate(r.left+r.width/2, r.top+r.height/2);
});
...
function runPicker(){
  ...
  function select(i){
    ...
    startPlaylistFrom(i);
    const load=line('<span class="dim">'+(pk.carregando||'carregando nós/oi-princesa')+'</span> ........ ');
  ...
  line('<span class="dim">'+tpl(pk.intro||'{n} faixas · escolhe a que abre a gente 🎵')+'</span>');
...
/* ---------- INGRESSO surpresa (Dunamis lacrada que ela revela) ---------- */
(function(){
...
function revealIngresso(){
  ...
  if(typeof unlockDunamisPin==='function') unlockDunamisPin();   // destrava o pino-segredo no mapa
  const r=card.getBoundingClientRect(); celebrate(r.left+r.width/2, r.top+r.height/2);
}
function initIngresso(){
  if(ingRevealed) return;
  if(ingTimer) clearTimeout(ingTimer);
  ingTimer=setTimeout(revealIngresso, 2600);   // failsafe: revela sozinho se ela não clicar
}
(function(){
...
/* ---------- DUNAMIS: contagem regressiva (config.dunamis) ---------- */
fill('#dunamisEyebrow', CONFIG.dunamis.eyebrow);
fill('#dunamisTitle', CONFIG.dunamis.titulo);
fill('#dunamisLegend', CONFIG.dunamis.legenda);
let dunamisTimer=null;
function startDunamis(){
  const grid=$('#dunamisGrid'); if(!grid) return;
  const alvo=new Date(CONFIG.dunamis.data).getTime();
  function tick(){
    let diff=alvo-Date.now();
    if(isNaN(alvo) || diff<=0){ grid.innerHTML=''; $('#dunamisLegend').textContent=CONFIG.dunamis.esgotado; if(dunamisTimer){ clearInterval(dunamisTimer); dunamisTimer=null; } return; }
    const d=Math.floor(diff/86400000); diff-=d*86400000;
    const h=Math.floor(diff/3600000); diff-=h*3600000;
    const mi=Math.floor(diff/60000); diff-=mi*60000;
    const s=Math.floor(diff/1000);
  ...
  tick(); if(dunamisTimer) clearInterval(dunamisTimer); dunamisTimer=setInterval(tick,1000);
}
...
function initSea(){
  ...
  function frame(t){
    ...
    // escurece quando estamos no fundo
    ctx.globalCompositeOperation='source-over';
    ctx.fillStyle='rgba(3,5,11,'+(0.5*(1-dep)).toFixed(3)+')'; ctx.fillRect(0,0,W,H);
    // brilho da superfície (mais forte conforme sobe)
    const top=ctx.createLinearGradient(0,0,0,H*0.7);
    top.addColorStop(0,'rgba(86,148,196,'+(0.05+dep*0.22).toFixed(3)+')');
    ...
    if(seaMoon>0.01){
      ...
      ctx.globalCompositeOperation='lighter';
      for(const s of stars){ ctx.globalAlpha=(0.35+0.55*Math.abs(Math.sin(time*0.8+s.ph)))*seaMoon;
